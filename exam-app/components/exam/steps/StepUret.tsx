...
import { List, PenLine, ToggleLeft, Pencil, Sparkles, Lock } from 'lucide-react'
import { YaziliBicimi, BICIM_TIPLERI } from './StepYaziliBicimi'
...
const BICIM_LABEL: Record<YaziliBicimi, string> = {
  test: 'Test',
  klasik: 'Klasik Yazılı',
  karma: 'Karma',
}
...
export function StepUret({ kazanim, yaziliBicimi, onGenerated }: Props) {
  const allowedTypes = BICIM_TIPLERI[yaziliBicimi]
  const [types, setTypes] = useState<QuestionType[]>([allowedTypes[0]])
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Soru tipi — biçime göre kilitli/açık */}
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="flex items-center gap-1 text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
            ...
            {BICIM_LABEL[yaziliBicimi]}
          </span>
        </div>
        <div className="grid grid-cols-2 gap-2">
          {QUESTION_TYPES.map(({ value, label, description, icon }) => {
            const allowed = allowedTypes.includes(value)
            ...
            return (
              ...
              >
                <div className={!allowed ? 'text-gray-300' : selected ? 'text-blue-500' : 'text-gray-400'}>{icon}</div>
                <span className="text-xs font-semibold leading-tight">{label}</span>
                <span className={`text-xs ${!allowed ? 'text-gray-300' : selected ? 'text-blue-500' : 'text-gray-400'}`}>{description}</span>
              </button>
            )
          })}
        </div>
        ...
        <div className="flex gap-2">
          {(['kolay', 'orta', 'zor'] as DifficultyLevel[]).map(d => (
            <button
              ...
              className={`flex-1 py-2 rounded-xl border-2 text-sm font-medium transition-all focus:outline-none ${
                difficulty === d
